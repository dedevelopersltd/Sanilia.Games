...
const AddSocialLinks = ({
  ...
  const [socialLinks, setSocialLinks] = useState(
    userSocialLinks?.length
      ? [...userSocialLinks]
      : [{ socialName: "", link: "" }]
  );

  console.log("sd", userSocialLinks, "sl", socialLinks);
  ...
  const handleSubmit = async (e) => {
    // setLoading(true);
    e.preventDefault();
    setUserSocialLinks([...socialLinks]);
    onClose();
    // return
    // try {
                  ...
                  {socialLinks.map((socialLink, index) => (
                    <div key={index} className="mb-6 flex items-center">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleChangeSocialLink(index, "link", e.target.value)
                        }
                        required
                      />
                      {index == 0 || socialLinks.length === 0 ? (
                        <>
                          <button
                            type="button"
